...
class Webpage
{
    public:
        ...
        string getProxyAddr (void) const;      //获得代理地址
        string checkProxyOutIpByThirdparty (void) const;  //真正的对外ip
        string checkProxyOutRegionByThirdparty (void) const;//真正的对外区域

        string getUserAgent (void) const;   //获得用户客户端
        string checkUserAgentByThirdparty (void) const;

        const string& getTxt (void) const;
        const string& getTitle (void) const;

        long getLatestHttpStatusCode (void) const;  //拿到http码
        bool isValidLatestHttpStatusCode (void) const; //http码合法性判断
        ...
        bool setMultiPostSectionsList (const vector<pair<string, string>>& post_sections_list); //构造post
        ...
        const vector<string>& getCookies (void) const;  //拿cookie

    private:
        bool download_ ( const string& raw_url,      //url
                         const string& filename,
                         const string& referer,      //tell where i am from
    ...
    private:
        ...
        string requestHttpHeader_ ( const string& raw_url,      //拿到http头里的对应项，raw_url和url区别是url没有经过escape转化
    ...
    private:
        ...
        string proxy_addr_;
        string txt_;    //地址的html
        string title_;  //地址的title
        bool b_loaded_ok_;
        long latest_http_status_code_;
        double aver_speed_download_;
        const string user_agent_;
        vector<string> cookie_items_list_; //cookies
};
